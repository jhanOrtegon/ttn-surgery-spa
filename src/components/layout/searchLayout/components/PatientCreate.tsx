...
interface IVoluntaryInsurer {
	id: string;
	companyTypeTag?: 'prepaid' | 'special_agreement' | '';
...
export const PatientCreate = ({ className, onBack }: IPatientCreate) => {
	//= ================================ =//
	//= ========== 🚑 formik ========== =//
	//= ================================ =//
	...
	const insuranceObligatory: IVoluntaryInsurer = {
		id: nanoid(),
		isMain: 1,
						...
						<div ref={parent}>
								...
								>
									...
									<div className='grid gap-x-6 gap-y-4 justify-between grid-cols-2'>
										<CustomSelect
											...
											errorMessage={
												((touched as JSONObject)[String(`companyTypeTag_${voluntary.id}` || '')] ||
													touched?.contracts?.[index]?.companyTypeTag) &&
												(errors?.contracts?.[index] as unknown as IVoluntaryInsurer)?.companyTypeTag
											}
											isInvalid={hasData(
												((touched as JSONObject)[String(`companyTypeTag_${voluntary.id}` || '')] ||
													touched?.contracts?.[index]?.companyTypeTag) &&
													(errors?.contracts?.[index] as unknown as IVoluntaryInsurer)
														?.companyTypeTag,
											)}
										/>
										...
										<CustomSelect
											...
											isDisabled={!hasData(formik?.values?.contracts?.[index]?.companyTypeTag)}
											...
											errorMessage={
												((touched as JSONObject)[
													...
													touched?.contracts?.[index]?.corporateClientId) &&
												(errors?.contracts?.[index] as unknown as IVoluntaryInsurer)
													?.corporateClientId
											}
											isInvalid={hasData(
										...
										<CustomSelect
											...
											isDisabled={!hasData(formik?.values?.contracts?.[index]?.corporateClientId)}
											...
											errorMessage={
												((touched as JSONObject)[String(`contractId_${voluntary.id}` || '')] ||
													touched?.contracts?.[index]?.contractId) &&
												(errors?.contracts?.[index] as unknown as IVoluntaryInsurer)?.contractId
											}
											isInvalid={hasData(
												((touched as JSONObject)[String(`contractId_${voluntary.id}` || '')] ||
													touched?.contracts?.[index]?.contractId) &&
										...
										<CustomInput
											isRequired
											disabled={!hasData(formik?.values?.contracts?.[index]?.contractId)}
											name='policy'
											label='No. de Póliza'
											...
											errorMessage={
												((touched as JSONObject)[String('policy')] ||
													touched?.contracts?.[index]?.policy) &&
												(errors?.contracts?.[index] as unknown as IVoluntaryInsurer)?.policy
											}
											isInvalid={hasData(
												((touched as JSONObject)[String('policy')] ||
													touched?.contracts?.[index]?.policy) &&
							...
							{(formik?.values?.contracts?.length || 0) < voluntaryCompanyTypes.length - 1 && (
								<Button
									...
									onClick={() => {
										const newInsurers: IVoluntaryInsurer = {
											id: nanoid(),
											companyTypeTag: '',
											corporateClientId: 0,
											contractId: 0,
